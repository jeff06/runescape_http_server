...
// The UnlockFunc type is an adapter to allow the use of ordinary
// function as Unlock mutator.
type UnlockFunc func(context.Context, *ent.UnlockMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UnlockFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UnlockMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UnlockMutation", m)
}
